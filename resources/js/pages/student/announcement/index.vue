<template>
    <div>
        <v-card class="mb-4">
                ...
                <div class="pl-4">
                    <div v-if="!loading">
                        <template v-for="announce in announces">
                            <div style="font-size:21px;" :key="announce.id"><a href="#">#{{announce.title}}</a></div>
                        </template>
                    </div>
                    <div v-else>
                        <v-skeleton-loader
            ...
            <v-card-actions class="pt-0 mt-0" v-if="announces.length>2">
...
export default {
    ...
    methods: {
        getAnnounces() {
            axios.get(`/student-api/announces`).then(({data}) => {
                this.announces = data
                setTimeout(() => {
                    this.loading = false
                },2000)
